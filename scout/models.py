...
Bounds are declared on the models rather than checked in endpoint bodies, so
FastAPI publishes them in /openapi.json and an agent can read the limits
instead of discovering them through a 422. See docs/design/api.md.
"""
...
# Bump on any breaking change to Evidence/SearchResponse/IngestResponse.
...
# Stripped, so a whitespace-only query collapses to "" and fails min_length
# in the schema rather than in a hand-rolled endpoint check.
QueryStr = Annotated[
    str,
...
class IngestRequest(BaseModel):
    """Either `url` (Scout fetches it) or `html` + `source_url` (the caller
