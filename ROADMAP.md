...
- Auth (even a simple API-key header) before this is exposed beyond localhost. **Still the main blocker to public hosting.**
- Rate limiting on `/ingest`, since it does outbound fetches on the caller's behalf. Per-request size caps exist; per-caller rate limits do not.
...
- Not yet covered: authentication and per-caller rate limiting (Phase 3).
